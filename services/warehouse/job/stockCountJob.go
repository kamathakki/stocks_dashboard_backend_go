...
func RunJob() {
	countries, err := warehouseendpoints.GetCountries(nil, nil)
	if err != nil {
	...
	if isJobTime == true {
		for _, country := range countries {
			go func() {
				...
				reqAdd, _ := http.NewRequest(http.MethodPost, "/warehouse/addStockCountHistoryForCountry", bytes.NewReader(b))
