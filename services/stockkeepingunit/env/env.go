...
var EnvKeys = struct {
	...
	REFRESH_SECRET_KEY: "REFRESH_SECRET_KEY", SKU_PORT: "SKU_PORT", WAREHOUSE_HOST: "WAREHOUSE_HOST"}
...
func init() {
	...
	requiredEnvs := []string{"PG_USER", "PG_PASS", "DB_NAME", "SCHEMA_NAME", "BACKEND_PORT", "SECRET_KEY", "REFRESH_SECRET_KEY", "SKU_PORT", "WAREHOUSE_HOST"}

	for _, envVariable := range requiredEnvs {
		envVariableValue := os.Getenv(envVariable)

		if envVariableValue == "" {
			panic(fmt.Sprintf("Env variable %v is missinger", envVariable))
		}
	}
